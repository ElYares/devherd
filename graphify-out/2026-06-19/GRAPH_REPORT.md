...
## Corpus Check
- 126 files · ~66,168 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1263 nodes · 2271 edges · 80 communities (72 shown, 8 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 350 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d246271c`
...
Cohesion: 0.22
Nodes (20): manifestCompose, manifestProxy, Project, Command(), composeArgs(), Down(), DownProject(), joinOutput() (+12 more)
...
Cohesion: 0.09
Nodes (55): confirm(), ensureComposeOrScaffold(), newScaffoldCmd(), promptDatabase(), Command, T, packageJSON, Plan (+47 more)
...
Cohesion: 0.08
Nodes (54): browserCommand(), newOpenCmd(), TestBrowserCommand(), collectDomains(), newProxyApplyCmd(), newProxyBootstrapCmd(), newProxyCmd(), syncManagedDomains() (+46 more)
...
Cohesion: 0.20
Nodes (11): ProjectNameForPath (SHA1 stable name), Compose Container Isolation Pattern, DevHerd CLI Commands Reference, Current Project Status, DevHerd CLI (Go + Cobra), Desde donde ejecutar, DevHerd, Enfoque del MVP 1 (+3 more)
...
Cohesion: 0.09
Nodes (23): 10. Siguiente bloque recomendado, 1. Que ya funciona, 2. Que hace hoy DevHerd, 3. Que ya validamos en un host real, 4. Que ya validamos manualmente con `local_proxy`, 5. Lo que ya quedo automatizado en `2026-05-04`, 6. Lo que ya validamos en un stack sensible real el `2026-05-04`, 7. Validacion operativa de `aang-server` y `Uniformes` (+15 more)
...
Cohesion: 0.07
Nodes (44): externalSettingsConfig, Config, T, Context, Duration, T, Context, Paths (+36 more)
...
Cohesion: 0.17
Nodes (12): 11. Comandos y como funcionan, 12. Criterio de exito del MVP, 1. Objetivo, 2. Principio de aislamiento, 4. Ingestion, 5. Normalizacion, 6. Agrupacion de issues, 7. Correlacion con contenedores (+4 more)
...
Cohesion: 0.17
Nodes (10): loadAppContext(), appContext, newListCmd(), newStopCmd(), Config, Context, DB, Paths (+2 more)
...
Cohesion: 0.18
Nodes (11): 10. Desactivar Observe en el proyecto, 10. Flujo de uso en un proyecto, 1. Arrancar el collector local, 2. Revisar el DSN local del proyecto, 3. Generar el override Compose local, 4. Levantar el proyecto, 5. Capturar una falla, 6. Inspeccionar desde CLI (+3 more)
...
Cohesion: 0.22
Nodes (9): Archivos generados, Bases de datos y Redis, Detección fina, Flags, Oferta automática desde `up`, Scaffolding de Docker para repos sin contenedores, Sin colisiones con tus proyectos levantados, Stacks soportados (+1 more)
...
Cohesion: 0.33
Nodes (6): 9. Fases de implementacion, Fase 1: Collector y persistencia local, Fase 2: Attach por proyecto, Fase 3: Correlacion con Docker, Fase 4: Panel local, Fase 5: Alertas
...
Cohesion: 0.50
Nodes (4): 3. Componentes, Base de datos, CLI, Collector local
...
## Knowledge Gaps
- **405 isolated node(s):** `name`, `version`, `minAppVersion`, `description`, `author` (+400 more)
...
- **Why does `String()` connect `observe migrations` to `Preflight & Inspection`, `Compose Runtime & Project Naming`, `Community 66`, `Doctor Host Checks`, `CLI Commands & Root`, `Observe Store & SQLite Models`, `Observe HTTP Server & Panel`, `External Proxy Connection`, `Domain & Projects DB`, `Project Stack Detector`, `Docker CLI Runtime`, `Compose Observe Override`, `Observe DB Manager`, `Community 55`, `Community 60`?**
  _High betweenness centrality (0.344) - this node is a cross-community bridge._
- **Why does `BuildComposeOverride()` connect `Compose Observe Override` to `observe migrations`, `Compose Override Tests`, `Observe CLI Commands`?**
...
- **Are the 13 inferred relationships involving `Detect()` (e.g. with `ensureComposeOrScaffold()` and `newScaffoldCmd()`) actually correct?**
  _`Detect()` has 13 INFERRED edges - model-reasoned connections that need verification._
